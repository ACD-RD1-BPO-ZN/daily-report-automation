...
async def generate_daily_report():
    ...
    **📢 【今日頭條】**
    {headline_img_tag}
    (🚨頭條規則🚨：頭條只能是「一篇」最具影響力的新聞，用 2~3 句話深入摘要其核心重點。[資料來源] 下方只放「一條」對應的網址連結，絕對不可以放多條。)
    [資料來源]
    ---
    **🎨 【引擎相關】**
    {ta_img_tag}
    ...
    **🌌 【今日全方位深度總結】**
    ...
    🚨🚨【強制條列格式】🚨🚨：每一則獨立的新聞「必須」是一個獨立的 bullet point（以 `- ` 開頭），嚴禁把多則新聞合併成同一個段落。每個引擎子標題下的每一篇文章都用 `- ` 開頭獨立成行，每行 1~2 句摘要。範例：
    - **Unreal Engine**：
      - 第一篇文章的摘要。
      - 第二篇文章的摘要。
    - **Unity**：
