...
async def generate_daily_report():
    ...
    **📢 【今日頭條】**
    {headline_img_tag}
    (🚨頭條規則🚨：頭條只能是「一篇」最具影響力的新聞，用 2~3 句話深入摘要其核心重點。摘要結束後的「下一行」，必須另起一個子項目放置來源連結，例如：`- [標題](<網址>)`。)
    ---
    **⚙️ 【引擎相關】**
    {engine_img_tag}
    ...
    **每篇原始文章（以 URL 為唯一識別）在整份報告中只能出現「一次」，只能歸屬在「一個」最適合的板塊中。**
    - 一旦某篇文章（URL）被【今日頭條】使用，【TA相關】、【引擎相關】等任何其他板塊皆**不得再引用同一篇文章的同一個URL**。
    - 引擎開發者訪談（如 UE5 遊戲開發案例）應歸入【引擎相關】，**不應重複出現在【獨立遊戲】**。
    - 同理，出現在【引擎相關 > Unity】的文章，**不應再出現於【獨立遊戲】或【製作人週記】**。
